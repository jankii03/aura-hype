...
export const Route = createFileRoute("/brands/dior/")({
	component: DiorPage,
	loader: async ({ context }) => {
		...
		await context.queryClient.prefetchQuery(
			context.trpc.products.list.queryOptions({ brand: "Dior" }),
		);
	},
});
...
function DiorPage() {
	...
	const { data } = useQuery(
		trpc.products.list.queryOptions({ brand: "Dior", ...(genderFilter && { gender: genderFilter }) })
	);
	...
	return (
		<div className="min-h-screen text-white relative">
			...
			<main className="relative z-10 container mx-auto px-6 py-12 pt-28">
				<h1 className="text-4xl md:text-6xl font-bold mb-8 uppercase tracking-wider">Dior</h1>
