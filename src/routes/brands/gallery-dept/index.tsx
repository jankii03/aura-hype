...
export const Route = createFileRoute("/brands/gallery-dept/")({
	component: GalleryDeptPage,
	loader: async ({ context }) => {
		if (isCloudflare()) {
			return;
		}
		await context.queryClient.prefetchQuery(
			context.trpc.products.list.queryOptions({ brand: "Gallery Dept." }),
		);
	},
});

function GalleryDeptPage() {
	...
	const { data } = useQuery(
		trpc.products.list.queryOptions({ brand: "Gallery Dept.", ...(genderFilter && { gender: genderFilter }) })
	);
	...
	return (
		<div className="min-h-screen text-white relative">
			...
			<main className="relative z-10 container mx-auto px-6 py-12 pt-28">
				<h1 className="text-4xl md:text-6xl font-bold mb-8 uppercase tracking-wider">Gallery Dept.</h1>
